...
| **G2** | Keys | Mints this node's own Ed25519 keypair and its operator root. Offers a storage menu (hardware token, TPM, OS credential store, or a passphrase-wrapped file) — the private key never leaves the machine and never enters any repository. | **Yes.** You choose the root ceremony and the storage medium. | Freely, until the operator root signs its first artifact; after that, only by a superseding root with recorded lineage — never a silent replacement. |
