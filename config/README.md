...
| `routing-policy.yaml` | Which task SHAPE resolves to which tier, which provider-agnostic model FAMILIES fill that tier, which pools can serve them, and -- per estate ring -- which provider CLASSES a payload may cross to at all. Ships `pools.entries: []`, the strictest fence (`local` only, every ring), and a fictional worked example loaded only on request. Names no vendor, no product and no model id. | **Yes** for anything that routes: an undeclared task shape, an undeclared estate ring, a pool missing a load-bearing field, or a declared ring with no egress rule each HALT the loader. A node with no routing policy routes nothing; it does not route everywhere. Absent the file, nothing else in the gate chain is affected. |
...
| `loop-charters.template.yaml` | The ONE declaration of every unattended cadence a node runs -- purpose, cadence, tier ceiling (capped at T2), budget share, kill switch and the evidence each loop owes. Host scheduling artifacts (a cron line, a systemd timer, a scheduled-task XML) are RENDERED from it by an adapter and never authored by hand. Ships `entries: []` with a commented, fictional example; every charter is born `enabled: false`. | **Yes** once a node runs loops: `intentops loops check` and every adapter HALT on a missing charter file, because "this node runs no loops" and "the packaging dropped the file" are different facts. `entries: []` is the valid, honest empty state. An undeclared field, a missing load-bearing field, an absolute kill switch, or a ceiling above T2 each HALT the loader. |
